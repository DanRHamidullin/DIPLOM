...
class Invoice(models.Model):
    ...
    def __str__(self):
        return '%s %s' % (self._id, self.sum)
        # return self._id
    # __str__()Метод вызывается всякий раз, когда вы вызываете str() объект. Джанго используется
...
class Zadanie2(models.Model):
    user_name = models.TextField(null=True, blank=True)
    user_phone = models.CharField(max_length=200, null=True, blank=True)
    user_date = models.DateTimeField(null=True, blank=True)
    user_email = models.CharField(max_length=200, null=True, blank=True)

    def __str__(self):
        return str(self.user_name,self.user_phone,self.user_date,self.user_email)
    # def __str__(self):
    #     return '%s %s' % (self.user_name)